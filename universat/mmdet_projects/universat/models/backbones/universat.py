"""MMDetection-compatible backbone wrapper for UniverSat.

This file is a migration template for OpenMMLab projects. Place the whole
``universat/`` project folder under ``mmdetection/projects/`` (or the equivalent
location in MMSeg/MMRotate) and run training with the provided configs.
...
# ---------------------------------------------------------------------------
# MMDet registration (graceful fallback for standalone testing)
# ---------------------------------------------------------------------------
try:
...
def _unroll_block_list(blocks: List[str]) -> List[str]:
    """Expand block repeat shorthand such as ``"SAx12"``."""
    out = []
    for b in blocks:
        if "x" in b:
            n = int(b.split("x")[1])
...
# ---------------------------------------------------------------------------
# Default modality dict used by the released pretrained weights.
# ---------------------------------------------------------------------------
...
class UniverSatBackbone(BaseModule):
    """MMDet backbone wrapping the UniverSat multimodal encoder.

    Args:
        modalities (list[str]): Modality names to encode. Each name must be
            present in ``modality_registry`` or you must provide explicit
            ``wavelengths`` / ``input_res`` / ``subpatches`` overrides.
        embed_dim (int): Trunk embedding dimension. Default 768.
        num_heads (int): Number of attention heads in the trunk. Default 12.
        mlp_ratio (float): MLP hidden expansion ratio. Default 4.0.
        patch_size (float): Physical patch size in **meters**. Default 40.
        output_grid (int | None): Side length ``G`` of the desired ``G x G``
            output feature map. If ``None``, defaults to finest sub-patch
            resolution inferred from inputs.
        block_type (list[str]): Ordered trunk block types. Default matches the
            released Base checkpoint.
        n_registers (int): Number of learnable register tokens. Default 4.
        gating (bool): Enable sigmoid gating in attention layers. Default True.
        proba_drop_modalities (float): Modality dropout probability. Set to
            ``0.0`` for inference/finetuning. Default 0.0.
        modalities_dict (dict | None): ``{dataset: [modality, ...]}`` mapping
            used to build SSL projector heads. Must match the pretrained
            checkpoint if projector weights are loaded; otherwise ignored.
        wavelengths (dict | None): Override modality wavelengths.
        input_res (dict | None): Override physical resolution in meters/pixel.
        subpatches (dict | None): Override sub-patch factor.
        keep_intermediate (bool): Whether to expose per-modality projector
            outputs. Default False.
        out_indices (tuple): MMDet-style output indices. Currently single
            scale, so only ``(-1,)`` is fully supported.
        multi_grids (list[int] | None): If set, run the trunk multiple times
            with different output side lengths and return all as multi-scale
            features. Useful for FPN-style necks.
        init_cfg (dict | None): MMDet init config. ``checkpoint`` can be a
            HuggingFace ``model.safetensors`` or a PyTorch ``.pth`` / ``.ckpt``.
    """
    ...
    def __init__(
        ...
        if BaseModule is nn.Module:
            # MMDet not installed: ignore init_cfg.
            super().__init__()
        ...
        # Resolve per-modality metadata from registry or user overrides.
        self.wavelengths = {}
        self.input_res = {}
        self.subpatches = {}
        ...
        # Build UPE as a partial so UniverSat injects its norm_layer.
    ...
    def init_weights(self) -> None:
        ...
        # Ignore SSL projector heads when using UniverSat as a backbone.
    ...
    def forward(self, x: Union[torch.Tensor, Dict[str, torch.Tensor]]) -> List[torch.Tensor]:
        ...
        Args:
            x: Either a dict ``{modality: tensor}`` or a single tensor that
                will be assigned to the first modality.
